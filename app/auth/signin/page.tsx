...
export default function SignInPage() {
  ...
  useEffect(() => {
    ...
    if (verified === "true") {
      setMessage({ status: "success", text: "Email verified successfully! You can now sign in." });
    } else if (error === "invalid_token") {
      setMessage({ status: "error", text: "Invalid verification link. Please request a new one." });
    } else if (error === "expired_token") {
      setMessage({ status: "error", text: "Verification link expired. Please request a new one." });
    } else if (error === "user_not_found") {
      setMessage({ status: "error", text: "User not found. Please sign up again." });
    } else if (error === "verification_failed") {
      setMessage({ status: "error", text: "Verification failed. Please try again." });
    ...
    try {
      ...
      if (result?.error) {
        ...
        if (result.error.includes("verify your email")) {
          setMessage({ status: "error", text: "Please verify your email before signing in. Check your inbox for the verification link." });
        } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#1a1a1a] rounded-lg shadow-xl p-8 border border-[#333]">
          ...
          <button
            ...
            className="w-full bg-[#0a0a0a] hover:bg-black border border-[#333] text-white font-medium py-3 rounded-lg transition-all flex items-center justify-center gap-2"
          >
        ...
        <div className="text-center mt-6 space-y-2">
          ...
          <p className="text-gray-400 text-sm">
            ...
            <a href="/auth/resend-verification" className="text-blue-500 hover:text-blue-400 font-medium">
              Resend
            </a>
          </p>
        </div>
